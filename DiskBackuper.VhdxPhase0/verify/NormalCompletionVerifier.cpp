...
namespace diskbackuper::vhdx_phase0
{
        ...
        std::error_code& error)
    {
        ...
        if (copyBlockSize == 0 ||
            copyBlockSize % logicalSectorSize != 0 ||
        ...
        std::vector<std::byte> sourceBlock(copyBlockSize);
...

        {
            ...
            std::vector<std::byte> destinationBlock(copyBlockSize);
            offset = 0;
            while (offset < result.logicalSize)
            {
                const std::size_t currentBlockSize = static_cast<std::size_t>(
        ...
        return GetFileSize(outputPath, result.vhdxFileSize, error);
    }
}
